...
abstract contract MainnetTests is DssTest {
    ...
    IALMProxy              almProxy;
    IControllerFull mainnetController;
    ...
    function setUp() public virtual {
        ...
        mainnetController = IControllerFull(
    ...
    function _wireAaveV4Facet() internal {
        ...
        wires[0] = IEnumerableIntegrations.Wire(
            IControllerFull.aaveV4_setMaxSlippage.selector,
            IAaveV4Facet.setMaxSlippage.selector
        );

        wires[1] = IEnumerableIntegrations.Wire(
            IControllerFull.aaveV4_getMaxSlippage.selector,
            IAaveV4Facet.getMaxSlippage.selector
        );

        wires[2] = IEnumerableIntegrations.Wire(
            IControllerFull.aaveV4_deposit.selector,
            IAaveV4Facet.deposit.selector
        );

        wires[3] = IEnumerableIntegrations.Wire(
            IControllerFull.aaveV4_withdraw.selector,
            IAaveV4Facet.withdraw.selector
        );

        wires[4] = IEnumerableIntegrations.Wire(
            IControllerFull.aaveV4_getDepositRateLimitKey.selector,
            IAaveV4Facet.getDepositRateLimitKey.selector
        );

        wires[5] = IEnumerableIntegrations.Wire(
            IControllerFull.aaveV4_getWithdrawRateLimitKey.selector,
            IAaveV4Facet.getWithdrawRateLimitKey.selector
        );

        wires[6] = IEnumerableIntegrations.Wire(
            IControllerFull.aaveV4_VERSION.selector,
            IFacet.VERSION.selector
        );
    ...
    function _wireBasinFacet() internal {
        ...
        wires[0] = IEnumerableIntegrations.Wire(
            IControllerFull.basin_deposit.selector,
            IBasinFacet.deposit.selector
        );

        wires[1] = IEnumerableIntegrations.Wire(
            IControllerFull.basin_withdraw.selector,
            IBasinFacet.withdraw.selector
        );

        wires[2] = IEnumerableIntegrations.Wire(
            IControllerFull.basin_getDepositRateLimitKey.selector,
            IBasinFacet.getDepositRateLimitKey.selector
        );

        wires[3] = IEnumerableIntegrations.Wire(
            IControllerFull.basin_getWithdrawRateLimitKey.selector,
            IBasinFacet.getWithdrawRateLimitKey.selector
        );

        wires[4] = IEnumerableIntegrations.Wire(
            IControllerFull.basin_VERSION.selector,
            IFacet.VERSION.selector
        );
    ...
    function _wireCurveFacet() internal {
        ...
        wires[0] = IEnumerableIntegrations.Wire(
            IControllerFull.curve_setMaxSlippage.selector,
            ICurveFacet.setMaxSlippage.selector
        );

        wires[1] = IEnumerableIntegrations.Wire(
            IControllerFull.curve_getMaxSlippage.selector,
            ICurveFacet.getMaxSlippage.selector
        );

        wires[2] = IEnumerableIntegrations.Wire(
            IControllerFull.curve_swap.selector,
            ICurveFacet.swap.selector
        );

        wires[3] = IEnumerableIntegrations.Wire(
            IControllerFull.curve_addLiquidity.selector,
            ICurveFacet.addLiquidity.selector
        );

        wires[4] = IEnumerableIntegrations.Wire(
            IControllerFull.curve_removeLiquidity.selector,
            ICurveFacet.removeLiquidity.selector
        );

        wires[5] = IEnumerableIntegrations.Wire(
            IControllerFull.curve_getAggregateDepositRateLimitKey.selector,
            ICurveFacet.getAggregateDepositRateLimitKey.selector
        );

        wires[6] = IEnumerableIntegrations.Wire(
            IControllerFull.curve_getAssetDepositRateLimitKey.selector,
            ICurveFacet.getAssetDepositRateLimitKey.selector
        );

        wires[7] = IEnumerableIntegrations.Wire(
            IControllerFull.curve_getSwapRateLimitKey.selector,
            ICurveFacet.getSwapRateLimitKey.selector
        );

        wires[8] = IEnumerableIntegrations.Wire(
            IControllerFull.curve_getAggregateWithdrawRateLimitKey.selector,
            ICurveFacet.getAggregateWithdrawRateLimitKey.selector
        );

        wires[9] = IEnumerableIntegrations.Wire(
            IControllerFull.curve_getAssetWithdrawRateLimitKey.selector,
            ICurveFacet.getAssetWithdrawRateLimitKey.selector
        );

        wires[10] = IEnumerableIntegrations.Wire(
            IControllerFull.curve_VERSION.selector,
            IFacet.VERSION.selector
        );
    ...
    function _wireCCTPFacet() internal {
        ...
        wires[0] = IEnumerableIntegrations.Wire(
            IControllerFull.cctp_setDomainParameters.selector,
            ICCTPFacet.setDomainParameters.selector
        );

        wires[1] = IEnumerableIntegrations.Wire(
            IControllerFull.cctp_transfer.selector,
            ICCTPFacet.transfer.selector
        );

        wires[2] = IEnumerableIntegrations.Wire(
            IControllerFull.cctp_toCCTPRateLimitKey.selector,
            ICCTPFacet.toCCTPRateLimitKey.selector
        );

        wires[3] = IEnumerableIntegrations.Wire(
            IControllerFull.cctp_getDomainParameters.selector,
            ICCTPFacet.getDomainParameters.selector
        );

        wires[4] = IEnumerableIntegrations.Wire(
            IControllerFull.cctp_getToDomainRateLimitKey.selector,
            ICCTPFacet.getToDomainRateLimitKey.selector
        );

        wires[5] = IEnumerableIntegrations.Wire(
            IControllerFull.cctp_VERSION.selector,
            IFacet.VERSION.selector
        );

        wires[6] = IEnumerableIntegrations.Wire(
            IControllerFull.cctp_DESTINATION_CALLER.selector,
            ICCTPFacet.DESTINATION_CALLER.selector
        );

        wires[7] = IEnumerableIntegrations.Wire(
            IControllerFull.cctp_MIN_FINALITY_THRESHOLD.selector,
            ICCTPFacet.MIN_FINALITY_THRESHOLD.selector
        );

        wires[8] = IEnumerableIntegrations.Wire(
            IControllerFull.cctp_cctp.selector,
            ICCTPFacet.cctp.selector
        );

        wires[9] = IEnumerableIntegrations.Wire(
            IControllerFull.cctp_usdc.selector,
    ...
    function _wireDAIUSDSFacet() internal {
        ...
        wires[0] = IEnumerableIntegrations.Wire(
            IControllerFull.daiUSDS_swapUSDSToDAI.selector,
            IDAIUSDSFacet.swapUSDSToDAI.selector
        );

        wires[1] = IEnumerableIntegrations.Wire(
            IControllerFull.daiUSDS_swapDAIToUSDS.selector,
            IDAIUSDSFacet.swapDAIToUSDS.selector
        );

        wires[2] = IEnumerableIntegrations.Wire(
            IControllerFull.daiUSDS_daiToUSDSSwapRateLimitKey.selector,
            IDAIUSDSFacet.daiToUSDSSwapRateLimitKey.selector
        );

        wires[3] = IEnumerableIntegrations.Wire(
            IControllerFull.daiUSDS_usdsToDAISwapRateLimitKey.selector,
            IDAIUSDSFacet.usdsToDAISwapRateLimitKey.selector
        );

        wires[4] = IEnumerableIntegrations.Wire(
            IControllerFull.daiUSDS_VERSION.selector,
            IFacet.VERSION.selector
        );

        wires[5] = IEnumerableIntegrations.Wire(
            IControllerFull.daiUSDS_dai.selector,
            IDAIUSDSFacet.dai.selector
        );

        wires[6] = IEnumerableIntegrations.Wire(
            IControllerFull.daiUSDS_daiUSDS.selector,
            IDAIUSDSFacet.daiUSDS.selector
        );

        wires[7] = IEnumerableIntegrations.Wire(
            IControllerFull.daiUSDS_usds.selector,
    ...
    function _wireNFATHaloFacet() internal {
        ...
        wires[0] = IEnumerableIntegrations.Wire(
            IControllerFull.nfatHalo_setMaxAnnualGrowthRate.selector,
            INFATHaloFacet.setMaxAnnualGrowthRate.selector
        );

        wires[1] = IEnumerableIntegrations.Wire(
            IControllerFull.nfatHalo_issue.selector,
            INFATHaloFacet.issue.selector
        );

        wires[2] = IEnumerableIntegrations.Wire(
            IControllerFull.nfatHalo_repayPrincipal.selector,
            INFATHaloFacet.repayPrincipal.selector
        );

        wires[3] = IEnumerableIntegrations.Wire(
            IControllerFull.nfatHalo_repayInterest.selector,
            INFATHaloFacet.repayInterest.selector
        );

        wires[4] = IEnumerableIntegrations.Wire(
            IControllerFull.nfatHalo_getMaxAnnualGrowthRate.selector,
            INFATHaloFacet.getMaxAnnualGrowthRate.selector
        );

        wires[5] = IEnumerableIntegrations.Wire(
            IControllerFull.nfatHalo_getFacilityState.selector,
            INFATHaloFacet.getFacilityState.selector
        );

        wires[6] = IEnumerableIntegrations.Wire(
            IControllerFull.nfatHalo_getPosition.selector,
            INFATHaloFacet.getPosition.selector
        );

        wires[7] = IEnumerableIntegrations.Wire(
            IControllerFull.nfatHalo_getCurrentMaxOutstandingInterest.selector,
            INFATHaloFacet.getCurrentMaxOutstandingInterest.selector
        );

        wires[8] = IEnumerableIntegrations.Wire(
            IControllerFull.nfatHalo_getIssueRateLimitKey.selector,
            INFATHaloFacet.getIssueRateLimitKey.selector
        );

        wires[9] = IEnumerableIntegrations.Wire(
            IControllerFull.nfatHalo_getRepayInterestRateLimitKey.selector,
            INFATHaloFacet.getRepayInterestRateLimitKey.selector
        );

        wires[10] = IEnumerableIntegrations.Wire(
            IControllerFull.nfatHalo_getRepayPrincipalRateLimitKey.selector,
            INFATHaloFacet.getRepayPrincipalRateLimitKey.selector
        );

        wires[11] = IEnumerableIntegrations.Wire(
            IControllerFull.nfatHalo_VERSION.selector,
            IFacet.VERSION.selector
        );
    ...
    function _wireNFATPrimeFacet() internal {
        ...
        wires[0] = IEnumerableIntegrations.Wire(
            IControllerFull.nfatPrime_subscribe.selector,
            INFATPrimeFacet.subscribe.selector
        );

        wires[1] = IEnumerableIntegrations.Wire(
            IControllerFull.nfatPrime_withdraw.selector,
            INFATPrimeFacet.withdraw.selector
        );

        wires[2] = IEnumerableIntegrations.Wire(
            IControllerFull.nfatPrime_collect.selector,
            INFATPrimeFacet.collect.selector
        );

        wires[3] = IEnumerableIntegrations.Wire(
            IControllerFull.nfatPrime_getSubscribeRateLimitKey.selector,
            INFATPrimeFacet.getSubscribeRateLimitKey.selector
        );

        wires[4] = IEnumerableIntegrations.Wire(
            IControllerFull.nfatPrime_getCollectRateLimitKey.selector,
            INFATPrimeFacet.getCollectRateLimitKey.selector
        );

        wires[5] = IEnumerableIntegrations.Wire(
            IControllerFull.nfatPrime_getWithdrawRateLimitKey.selector,
            INFATPrimeFacet.getWithdrawRateLimitKey.selector
        );

        wires[6] = IEnumerableIntegrations.Wire(
            IControllerFull.nfatPrime_VERSION.selector,
            IFacet.VERSION.selector
        );
    ...
    function _wireERC7540Facet() internal {
        ...
        wires[0] = IEnumerableIntegrations.Wire(
            IControllerFull.erc7540_requestDeposit.selector,
            IERC7540Facet.requestDeposit.selector
        );

        wires[1] = IEnumerableIntegrations.Wire(
            IControllerFull.erc7540_claimDeposit.selector,
            IERC7540Facet.claimDeposit.selector
        );

        wires[2] = IEnumerableIntegrations.Wire(
            IControllerFull.erc7540_requestRedeem.selector,
            IERC7540Facet.requestRedeem.selector
        );

        wires[3] = IEnumerableIntegrations.Wire(
            IControllerFull.erc7540_claimRedeem.selector,
            IERC7540Facet.claimRedeem.selector
        );

        wires[4] = IEnumerableIntegrations.Wire(
            IControllerFull.erc7540_getRequestDepositRateLimitKey.selector,
            IERC7540Facet.getRequestDepositRateLimitKey.selector
        );

        wires[5] = IEnumerableIntegrations.Wire(
            IControllerFull.erc7540_getClaimDepositRateLimitKey.selector,
            IERC7540Facet.getClaimDepositRateLimitKey.selector
        );

        wires[6] = IEnumerableIntegrations.Wire(
            IControllerFull.erc7540_getRequestRedeemRateLimitKey.selector,
            IERC7540Facet.getRequestRedeemRateLimitKey.selector
        );

        wires[7] = IEnumerableIntegrations.Wire(
            IControllerFull.erc7540_getClaimRedeemRateLimitKey.selector,
            IERC7540Facet.getClaimRedeemRateLimitKey.selector
        );

        wires[8] = IEnumerableIntegrations.Wire(
            IControllerFull.erc7540_VERSION.selector,
            IFacet.VERSION.selector
        );
    ...
    function _wireEthenaFacet() internal {
        ...
        wires[0] = IEnumerableIntegrations.Wire(
            IControllerFull.ethena_setDelegatedSigner.selector,
            IEthenaFacet.setDelegatedSigner.selector
        );

        wires[1] = IEnumerableIntegrations.Wire(
            IControllerFull.ethena_removeDelegatedSigner.selector,
            IEthenaFacet.removeDelegatedSigner.selector
        );

        wires[2] = IEnumerableIntegrations.Wire(
            IControllerFull.ethena_prepareMint.selector,
            IEthenaFacet.prepareMint.selector
        );

        wires[3] = IEnumerableIntegrations.Wire(
            IControllerFull.ethena_prepareBurn.selector,
            IEthenaFacet.prepareBurn.selector
        );

        wires[4] = IEnumerableIntegrations.Wire(
            IControllerFull.ethena_cooldownAssets.selector,
            IEthenaFacet.cooldownAssets.selector
        );

        wires[5] = IEnumerableIntegrations.Wire(
            IControllerFull.ethena_cooldownShares.selector,
            IEthenaFacet.cooldownShares.selector
        );

        wires[6] = IEnumerableIntegrations.Wire(
            IControllerFull.ethena_unstake.selector,
            IEthenaFacet.unstake.selector
        );

        wires[7] = IEnumerableIntegrations.Wire(
            IControllerFull.ethena_setDelegatedSignerRateLimitKey.selector,
            IEthenaFacet.setDelegatedSignerRateLimitKey.selector
        );

        wires[8] = IEnumerableIntegrations.Wire(
            IControllerFull.ethena_removeDelegatedSignerRateLimitKey.selector,
            IEthenaFacet.removeDelegatedSignerRateLimitKey.selector
        );

        wires[9] = IEnumerableIntegrations.Wire(
            IControllerFull.ethena_mintRateLimitKey.selector,
            IEthenaFacet.mintRateLimitKey.selector
        );

        wires[10] = IEnumerableIntegrations.Wire(
            IControllerFull.ethena_burnRateLimitKey.selector,
            IEthenaFacet.burnRateLimitKey.selector
        );

        wires[11] = IEnumerableIntegrations.Wire(
            IControllerFull.ethena_cooldownRateLimitKey.selector,
            IEthenaFacet.cooldownRateLimitKey.selector
        );

        wires[12] = IEnumerableIntegrations.Wire(
            IControllerFull.ethena_unstakeRateLimitKey.selector,
            IEthenaFacet.unstakeRateLimitKey.selector
        );

        wires[13] = IEnumerableIntegrations.Wire(
            IControllerFull.ethena_VERSION.selector,
            IFacet.VERSION.selector
        );

        wires[14] = IEnumerableIntegrations.Wire(
            IControllerFull.ethena_minter.selector,
            IEthenaFacet.minter.selector
        );

        wires[15] = IEnumerableIntegrations.Wire(
            IControllerFull.ethena_susde.selector,
            IEthenaFacet.susde.selector
        );

        wires[16] = IEnumerableIntegrations.Wire(
            IControllerFull.ethena_usdc.selector,
            IEthenaFacet.usdc.selector
        );

        wires[17] = IEnumerableIntegrations.Wire(
            IControllerFull.ethena_usde.selector,
    ...
    function _wireFarmFacet() internal {
        ...
        wires[0] = IEnumerableIntegrations.Wire(
            IControllerFull.farm_deposit.selector,
            IFarmFacet.deposit.selector
        );

        wires[1] = IEnumerableIntegrations.Wire(
            IControllerFull.farm_claimReward.selector,
            IFarmFacet.claimReward.selector
        );

        wires[2] = IEnumerableIntegrations.Wire(
            IControllerFull.farm_withdraw.selector,
            IFarmFacet.withdraw.selector
        );

        wires[3] = IEnumerableIntegrations.Wire(
            IControllerFull.farm_getClaimRewardRateLimitKey.selector,
            IFarmFacet.getClaimRewardRateLimitKey.selector
        );

        wires[4] = IEnumerableIntegrations.Wire(
            IControllerFull.farm_getDepositRateLimitKey.selector,
            IFarmFacet.getDepositRateLimitKey.selector
        );

        wires[5] = IEnumerableIntegrations.Wire(
            IControllerFull.farm_getWithdrawRateLimitKey.selector,
            IFarmFacet.getWithdrawRateLimitKey.selector
        );

        wires[6] = IEnumerableIntegrations.Wire(
            IControllerFull.farm_VERSION.selector,
            IFacet.VERSION.selector
        );
    ...
    function _wireLayerZeroFacet() internal {
        ...
        wires[0] = IEnumerableIntegrations.Wire(
            IControllerFull.layerZero_setRecipient.selector,
            ILayerZeroFacet.setRecipient.selector
        );

        wires[1] = IEnumerableIntegrations.Wire(
            IControllerFull.layerZero_transfer.selector,
            ILayerZeroFacet.transfer.selector
        );

        wires[2] = IEnumerableIntegrations.Wire(
            IControllerFull.layerZero_getRecipient.selector,
            ILayerZeroFacet.getRecipient.selector
        );

        wires[3] = IEnumerableIntegrations.Wire(
            IControllerFull.layerZero_getTransferRateLimitKey.selector,
            ILayerZeroFacet.getTransferRateLimitKey.selector
        );

        wires[4] = IEnumerableIntegrations.Wire(
            IControllerFull.layerZero_quoteTransfer.selector,
            ILayerZeroFacet.quoteTransfer.selector
        );

        wires[5] = IEnumerableIntegrations.Wire(
            IControllerFull.layerZero_VERSION.selector,
            IFacet.VERSION.selector
        );
    ...
    function _wireMapleFacet() internal {
        ...
        wires[0] = IEnumerableIntegrations.Wire(
            IControllerFull.maple_requestRedemption.selector,
            IMapleFacet.requestRedemption.selector
        );

        wires[1] = IEnumerableIntegrations.Wire(
            IControllerFull.maple_cancelRedemption.selector,
            IMapleFacet.cancelRedemption.selector
        );

        wires[2] = IEnumerableIntegrations.Wire(
            IControllerFull.maple_getCancelRedeemRateLimitKey.selector,
            IMapleFacet.getCancelRedeemRateLimitKey.selector
        );

        wires[3] = IEnumerableIntegrations.Wire(
            IControllerFull.maple_getRequestRedeemRateLimitKey.selector,
            IMapleFacet.getRequestRedeemRateLimitKey.selector
        );

        wires[4] = IEnumerableIntegrations.Wire(
            IControllerFull.maple_VERSION.selector,
            IFacet.VERSION.selector
        );
    ...
    function _wireOTCFacet() internal {
        ...
        wires[0] = IEnumerableIntegrations.Wire(
            IControllerFull.otc_setMaxSlippage.selector,
            IOTCFacet.setMaxSlippage.selector
        );

        wires[1] = IEnumerableIntegrations.Wire(
            IControllerFull.otc_setBuffer.selector,
            IOTCFacet.setBuffer.selector
        );

        wires[2] = IEnumerableIntegrations.Wire(
            IControllerFull.otc_setRechargeRate.selector,
            IOTCFacet.setRechargeRate.selector
        );

        wires[3] = IEnumerableIntegrations.Wire(
            IControllerFull.otc_send.selector,
            IOTCFacet.send.selector
        );

        wires[4] = IEnumerableIntegrations.Wire(
            IControllerFull.otc_claim.selector,
            IOTCFacet.claim.selector
        );

        wires[5] = IEnumerableIntegrations.Wire(
            IControllerFull.otc_getBuffer.selector,
            IOTCFacet.getBuffer.selector
        );

        wires[6] = IEnumerableIntegrations.Wire(
            IControllerFull.otc_getMaxSlippage.selector,
            IOTCFacet.getMaxSlippage.selector
        );

        wires[7] = IEnumerableIntegrations.Wire(
            IControllerFull.otc_getRechargeRate.selector,
            IOTCFacet.getRechargeRate.selector
        );

        wires[8] = IEnumerableIntegrations.Wire(
            IControllerFull.otc_getState.selector,
            IOTCFacet.getState.selector
        );

        wires[9] = IEnumerableIntegrations.Wire(
            IControllerFull.otc_getClaimWithRecharge.selector,
            IOTCFacet.getClaimWithRecharge.selector
        );

        wires[10] = IEnumerableIntegrations.Wire(
            IControllerFull.otc_getIsSwapReady.selector,
            IOTCFacet.getIsSwapReady.selector
        );

        wires[11] = IEnumerableIntegrations.Wire(
            IControllerFull.otc_getSendRateLimitKey.selector,
            IOTCFacet.getSendRateLimitKey.selector
        );

        wires[12] = IEnumerableIntegrations.Wire(
            IControllerFull.otc_getClaimRateLimitKey.selector,
            IOTCFacet.getClaimRateLimitKey.selector
        );

        wires[13] = IEnumerableIntegrations.Wire(
            IControllerFull.otc_VERSION.selector,
            IFacet.VERSION.selector
        );
    ...
    function _wirePSMFacet() internal {
        ...
        wires[0] = IEnumerableIntegrations.Wire(
            IControllerFull.psm_swapUSDSToUSDC.selector,
            IPSMFacet.swapUSDSToUSDC.selector
        );

        wires[1] = IEnumerableIntegrations.Wire(
            IControllerFull.psm_swapUSDCToUSDS.selector,
            IPSMFacet.swapUSDCToUSDS.selector
        );

        wires[2] = IEnumerableIntegrations.Wire(
            IControllerFull.psm_to18ConversionFactor.selector,
            IPSMFacet.to18ConversionFactor.selector
        );

        wires[3] = IEnumerableIntegrations.Wire(
            IControllerFull.psm_usdcToUSDSSwapRateLimitKey.selector,
            IPSMFacet.usdcToUSDSSwapRateLimitKey.selector
        );

        wires[4] = IEnumerableIntegrations.Wire(
            IControllerFull.psm_usdsToUSDCSwapRateLimitKey.selector,
            IPSMFacet.usdsToUSDCSwapRateLimitKey.selector
        );

        wires[5] = IEnumerableIntegrations.Wire(
            IControllerFull.psm_VERSION.selector,
            IFacet.VERSION.selector
        );

        wires[6] = IEnumerableIntegrations.Wire(
            IControllerFull.psm_dai.selector,
            IPSMFacet.dai.selector
        );

        wires[7] = IEnumerableIntegrations.Wire(
            IControllerFull.psm_daiUSDS.selector,
            IPSMFacet.daiUSDS.selector
        );

        wires[8] = IEnumerableIntegrations.Wire(
            IControllerFull.psm_psm.selector,
            IPSMFacet.psm.selector
        );

        wires[9] = IEnumerableIntegrations.Wire(
            IControllerFull.psm_usdc.selector,
            IPSMFacet.usdc.selector
        );

        wires[10] = IEnumerableIntegrations.Wire(
            IControllerFull.psm_usds.selector,
    ...
    function _wireSuperstateFacet() internal {
        ...
        wires[0] = IEnumerableIntegrations.Wire(
            IControllerFull.superstate_subscribe.selector,
            ISuperstateFacet.subscribe.selector
        );

        wires[1] = IEnumerableIntegrations.Wire(
            IControllerFull.superstate_subscribeRateLimitKey.selector,
            ISuperstateFacet.subscribeRateLimitKey.selector
        );

        wires[2] = IEnumerableIntegrations.Wire(
            IControllerFull.superstate_VERSION.selector,
            IFacet.VERSION.selector
        );

        wires[3] = IEnumerableIntegrations.Wire(
            IControllerFull.superstate_usdc.selector,
            ISuperstateFacet.usdc.selector
        );

        wires[4] = IEnumerableIntegrations.Wire(
            IControllerFull.superstate_ustb.selector,
    ...
    function _wireUniswapV4Facet() internal {
        ...
        wires[0] = IEnumerableIntegrations.Wire(
            IControllerFull.uniswapV4_setMaxSlippage.selector,
            IUniswapV4Facet.setMaxSlippage.selector
        );

        wires[1] = IEnumerableIntegrations.Wire(
            IControllerFull.uniswapV4_setTickLimits.selector,
            IUniswapV4Facet.setTickLimits.selector
        );

        wires[2] = IEnumerableIntegrations.Wire(
            IControllerFull.uniswapV4_mintPosition.selector,
            IUniswapV4Facet.mintPosition.selector
        );

        wires[3] = IEnumerableIntegrations.Wire(
            IControllerFull.uniswapV4_increasePosition.selector,
            IUniswapV4Facet.increasePosition.selector
        );

        wires[4] = IEnumerableIntegrations.Wire(
            IControllerFull.uniswapV4_decreasePosition.selector,
            IUniswapV4Facet.decreasePosition.selector
        );

        wires[5] = IEnumerableIntegrations.Wire(
            IControllerFull.uniswapV4_swap.selector,
            IUniswapV4Facet.swap.selector
        );

        wires[6] = IEnumerableIntegrations.Wire(
            IControllerFull.uniswapV4_getAggregateDepositRateLimitKey.selector,
            IUniswapV4Facet.getAggregateDepositRateLimitKey.selector
        );

        wires[7] = IEnumerableIntegrations.Wire(
            IControllerFull.uniswapV4_getAssetDepositRateLimitKey.selector,
            IUniswapV4Facet.getAssetDepositRateLimitKey.selector
        );

        wires[8] = IEnumerableIntegrations.Wire(
            IControllerFull.uniswapV4_getMaxSlippage.selector,
            IUniswapV4Facet.getMaxSlippage.selector
        );

        wires[9] = IEnumerableIntegrations.Wire(
            IControllerFull.uniswapV4_getSwapRateLimitKey.selector,
            IUniswapV4Facet.getSwapRateLimitKey.selector
        );

        wires[10] = IEnumerableIntegrations.Wire(
            IControllerFull.uniswapV4_getTickLimits.selector,
            IUniswapV4Facet.getTickLimits.selector
        );

        wires[11] = IEnumerableIntegrations.Wire(
            IControllerFull.uniswapV4_getAggregateWithdrawRateLimitKey.selector,
            IUniswapV4Facet.getAggregateWithdrawRateLimitKey.selector
        );

        wires[12] = IEnumerableIntegrations.Wire(
            IControllerFull.uniswapV4_getAssetWithdrawRateLimitKey.selector,
            IUniswapV4Facet.getAssetWithdrawRateLimitKey.selector
        );

        wires[13] = IEnumerableIntegrations.Wire(
            IControllerFull.uniswapV4_VERSION.selector,
            IFacet.VERSION.selector
        );

        wires[14] = IEnumerableIntegrations.Wire(
            IControllerFull.uniswapV4_permit2.selector,
            IUniswapV4Facet.permit2.selector
        );

        wires[15] = IEnumerableIntegrations.Wire(
            IControllerFull.uniswapV4_positionManager.selector,
            IUniswapV4Facet.positionManager.selector
        );

        wires[16] = IEnumerableIntegrations.Wire(
            IControllerFull.uniswapV4_router.selector,
    ...
    function _wireUSDSFacet() internal {
        ...
        wires[0] = IEnumerableIntegrations.Wire(
            IControllerFull.usds_setVault.selector,
            IUSDSFacet.setVault.selector
        );

        wires[1] = IEnumerableIntegrations.Wire(
            IControllerFull.usds_mint.selector,
            IUSDSFacet.mint.selector
        );

        wires[2] = IEnumerableIntegrations.Wire(
            IControllerFull.usds_burn.selector,
            IUSDSFacet.burn.selector
        );

        wires[3] = IEnumerableIntegrations.Wire(
            IControllerFull.usds_vault.selector,
            IUSDSFacet.vault.selector
        );

        wires[4] = IEnumerableIntegrations.Wire(
            IControllerFull.usds_mintRateLimitKey.selector,
            IUSDSFacet.mintRateLimitKey.selector
        );

        wires[5] = IEnumerableIntegrations.Wire(
            IControllerFull.usds_burnRateLimitKey.selector,
            IUSDSFacet.burnRateLimitKey.selector
        );

        wires[6] = IEnumerableIntegrations.Wire(
            IControllerFull.usds_VERSION.selector,
            IFacet.VERSION.selector
        );

        wires[7] = IEnumerableIntegrations.Wire(
            IControllerFull.usds_usds.selector,
    ...
    function _wireWEETHFacet() internal {
        ...
        wires[0] = IEnumerableIntegrations.Wire(
            IControllerFull.weeth_deposit.selector,
            IWEETHFacet.deposit.selector
        );

        wires[1] = IEnumerableIntegrations.Wire(
            IControllerFull.weeth_requestWithdraw.selector,
            IWEETHFacet.requestWithdraw.selector
        );

        wires[2] = IEnumerableIntegrations.Wire(
            IControllerFull.weeth_claimWithdrawal.selector,
            IWEETHFacet.claimWithdrawal.selector
        );

        wires[3] = IEnumerableIntegrations.Wire(
            IControllerFull.weeth_getDepositRateLimitKey.selector,
            IWEETHFacet.getDepositRateLimitKey.selector
        );

        wires[4] = IEnumerableIntegrations.Wire(
            IControllerFull.weeth_getRequestWithdrawRateLimitKey.selector,
            IWEETHFacet.getRequestWithdrawRateLimitKey.selector
        );

        wires[5] = IEnumerableIntegrations.Wire(
            IControllerFull.weeth_getClaimWithdrawRateLimitKey.selector,
            IWEETHFacet.getClaimWithdrawRateLimitKey.selector
        );

        wires[6] = IEnumerableIntegrations.Wire(
            IControllerFull.weeth_VERSION.selector,
            IFacet.VERSION.selector
        );

        wires[7] = IEnumerableIntegrations.Wire(
            IControllerFull.weeth_weeth.selector,
            IWEETHFacet.weeth.selector
        );

        wires[8] = IEnumerableIntegrations.Wire(
            IControllerFull.weeth_weth.selector,
    ...
    function _wireWSTETHFacet() internal {
        ...
        wires[0] = IEnumerableIntegrations.Wire(
            IControllerFull.wsteth_deposit.selector,
            IWSTETHFacet.deposit.selector
        );

        wires[1] = IEnumerableIntegrations.Wire(
            IControllerFull.wsteth_requestWithdraw.selector,
            IWSTETHFacet.requestWithdraw.selector
        );

        wires[2] = IEnumerableIntegrations.Wire(
            IControllerFull.wsteth_claimWithdrawal.selector,
            IWSTETHFacet.claimWithdrawal.selector
        );

        wires[3] = IEnumerableIntegrations.Wire(
            IControllerFull.wsteth_depositRateLimitKey.selector,
            IWSTETHFacet.depositRateLimitKey.selector
        );

        wires[4] = IEnumerableIntegrations.Wire(
            IControllerFull.wsteth_requestWithdrawRateLimitKey.selector,
            IWSTETHFacet.requestWithdrawRateLimitKey.selector
        );

        wires[5] = IEnumerableIntegrations.Wire(
            IControllerFull.wsteth_claimWithdrawRateLimitKey.selector,
            IWSTETHFacet.claimWithdrawRateLimitKey.selector
        );

        wires[6] = IEnumerableIntegrations.Wire(
            IControllerFull.wsteth_VERSION.selector,
            IFacet.VERSION.selector
        );

        wires[7] = IEnumerableIntegrations.Wire(
            IControllerFull.wsteth_weth.selector,
            IWSTETHFacet.weth.selector
        );

        wires[8] = IEnumerableIntegrations.Wire(
            IControllerFull.wsteth_withdrawQueue.selector,
            IWSTETHFacet.withdrawQueue.selector
        );

        wires[9] = IEnumerableIntegrations.Wire(
            IControllerFull.wsteth_wsteth.selector,
